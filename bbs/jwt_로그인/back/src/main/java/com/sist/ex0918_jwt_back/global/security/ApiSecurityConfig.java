...
import com.sist.ex0918_jwt_back.domain.bbs.entity.controller.ApiBbsController;
...
public class ApiSecurityConfig {
    ...
    //final로 지정해야 RequiredArgsConstructor에 자동으로 대입됨
    ...
    public SecurityFilterChain apiFilterChain(HttpSecurity http) throws Exception{
        ...
        .headers(headers->headers.frameOptions(
            ...
            .securityMatcher("/api/**")//설정된 경로로 들어오는 모든것들 검사
            .authorizeRequests(
                authorize -> authorize
                ...
                //보안상 취약하여 테스트용 또는 프로토타입 같은 것에서 사용함
                //하지만 실제 서비스 JWT,OAuth2 세션 로그인법을 선호한다.
            ).formLogin(
                formLogin -> formLogin.disable()//폼 로그인 비활성화
            ).sessionManagement(
