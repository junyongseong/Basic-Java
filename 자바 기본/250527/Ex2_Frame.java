...
public class Ex2_Frame extends JFrame {
	...
	//2번째 작업=====================================
	Image meteor_img;//메테오 이미지
	ArrayList<Ex2_Meteor> m_list =new ArrayList<Ex2_Meteor>();
	boolean chk=true;
	...
	Thread makeMeteor = new Thread() {
		...
		public void run() {
			...
			while(chk) {
				...
				m_list.add(m);//생성된 운석 객체를 ArrayList에 저장(추가)
				m.start();//운석객체가 스레드 이므로 스스로 움직인다.
	...
	public Ex2_Frame() {
		...
		//2번째 작업에서 추가 화면이 보여진 이후 메테오 스레드 시작
		makeMeteor.start();
		...
		p.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				int step=5;
				switch(keyCode) {
				//me의rect안에 x값을 증/감 시킨다.
				case KeyEvent.VK_RIGHT://오른쪽
					/*me.rect.x +=5
					 * me.rect.x=Math.min(me.rect.x,p.getSize().width-me.rect.width)*/
					me.rect.x =Math.min(me.rect.x+step,p.getSize().width-me.rect.width );
					break;
