		...
		bk:while(true) {
			try {
				...
				if(obj !=null) {
					...
					case 3: 
					    server.removeClient(this); // 서버 유저 리스트에서 삭제
					    
					    Protocol updateProtocol = new Protocol();
					    updateProtocol.setCmd(1); // 명단 갱신해줘야함안하면 이르미그대로남음
					    updateProtocol.setUser_names(server.getNames());

					    server.sendProtocol(updateProtocol); //갱신된 명단 전송
					    break bk; // 스레드 종료
					case 1:
						//서버에 접속 한 경우
