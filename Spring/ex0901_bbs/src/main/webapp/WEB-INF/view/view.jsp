...
<div id="bbs">
  <form method="post" >
    <table summary="게시판 글쓰기">
      ...
      <c:if test="${vo.file_name ne null && vo.file_name.length > 4}">
      <tr>
        <th>첨부파일:</th>
...
<%--표현할 vo객체가 존재하지 않는다면 원래있던 목록페이지로 이동한다.--%>
<%--== 는 eq와 같음--%>
<c:if test="${requestScope.vo eq null}">
  <c:redirect url="/bbs/list">
...
<script>
  ...
  function goList() {
    // 요즘은 MZ들은 이렇게 쓴다더라
    location.href = "/bbs/list?cPage=" + document.ff.cPage.value;
