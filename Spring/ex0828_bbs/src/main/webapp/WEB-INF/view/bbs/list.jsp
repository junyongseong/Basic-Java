...
<div id="bbs">
    <table summary="게시판 목록">
                ...
                <ol class="paging">
                    ...
                    <c:forEach var="i" begin="${p.startPage}" end="${p.endPage}">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <li><a href="/bbs/list?cPage=${i}">${i}</a></li>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:choose>
        ...
        <c:forEach var="vo" items="${ar}" varStatus="i">
            <c:set var="num" value="${p.totalCount - ((p.nowPage - 1) * p.numPerPage + i.index)}" scope="page"/>
            <tr>
                <td>${num}</td>
                <td style="text-align: left">
                    <a href="/bbs/view?bname=BBS&b_idx=${vo.b_idx}&cPage=${p.nowPage}">
                            ${vo.subject}
                        <c:if test="${not empty vo.c_list}">
                            (${fn:length(vo.c_list)})
                        </c:if>
                    </a></td>
                <td>${vo.writer}</td>
