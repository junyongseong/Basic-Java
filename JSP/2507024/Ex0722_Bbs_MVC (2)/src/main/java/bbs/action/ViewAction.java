...
public class ViewAction implements Action{
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        //한번이라도 읽은 게시물들은 list에 담아서 HttpSession에 저장해둔다.
        //그럼 우선 HttpSession으로 부터 list를 얻어낸다.
        ...
        if (checkBbs(list,vo)) {
            BbsDAO.hit(b_idx);//조회수 증가하고
            list.add(vo);//list는 이미 HttpSession에 저장된 상태이므로
                    //이후의 별도 작업은 필요가 없다.
        }
