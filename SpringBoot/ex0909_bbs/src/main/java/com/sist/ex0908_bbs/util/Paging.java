...
public class Paging {
    ...
    int pagePerBlock = 5; // 한 블럭당 보여줄 페이지 수
    int totalPage; // 총 페이지 수(전체 페이지 수)
    ...
    private boolean isPrePage;//이전페이지 기능 가능여부(true: 이전 기능 활성화)
    private boolean isNextPage;//다음페이지 기능 가능여부(true: 다음 기능 활성화)

    //jsp에서 표현할 페이징 HTML코드를 저장하는 공간
    private StringBuffer pagingHTML;
    private String bname; // 게시판 이름

    //인자로 현재페이지,전체 게시물 수, 한페이지에 표현할 게시물의 수,
    // 한 블럭당 표현할 페이지 수를 받아서 생성함
    //JSP에서 표현할 페이징 HTML코드까지 만들고자 한다.
    public Paging(int nowPage, int totalCount,
     int numPerPage, int pagePerBlock,String bname) {
        ...
        //현재 페이지 값에 의해 시작게시물의 행번호와 마지막 게시물의 행번호 구하자
        ...
        //이전 페이지 기능 활성화 할 것인지 판단하자
        // 이전 페이지
// isPrePage = (startPage > 1) ? true : false;
// 다음 페이지
// isNextPage = (endPage < totalPage) ? true : false;
        isPrePage = startPage > 1;
        isNextPage = endPage < totalPage;
        //이제 현재 페이지 값도 알고 시작페이지와 마지막 페이지 값도 알게 되었으니
        ...
        for (int i = startPage; i <= endPage; i++) {
            if (i == nowPage) { //현재 페이지 값과 같은 페이지는 링크를 걸지 않는다.
